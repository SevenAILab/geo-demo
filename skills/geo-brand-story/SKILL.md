...
```json
{
  ...
  "valuePropOptions": [
    ...
    { "id": "vp-2", "label": "另一条价值主张", "suggested": false }
  ],
  "valueProps": ["vp-1"],
  "valuePropOther": "",
  "audience": "目标受众",
...
- `valuePropOptions` 2–6 条，每条 label 可执行、基于站点可见内容
- `suggested: true` 表示 AI 认为该价值主张适用于该品牌
- `valueProps` 初始为所有 `suggested: true` 的 id 列表
- `valuePropOther` 留空字符串，由用户在 UI 勾选「其他」后填写
- 缺口字段（valueProps, differentiator）必须基于 gaps 补全，不可留空占位
- 真优势需可验证：用户认 × 竞品无 × 可结构化表达
- competitors 1–3 个 URL 或空数组

## 语言

- JSON 中所有面向用户的字符串字段（含 `valuePropOptions.label`）必须使用**简体中文**
- 枚举/id 等技术字段保持英文
