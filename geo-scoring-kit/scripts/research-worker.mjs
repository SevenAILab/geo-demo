...
export function buildResearchQueryVariants(question, { retry = false, maxVariants = 3 } = {}) {
  ...
  const stopwords =
    /^(如何|哪些|什么|为什么|是否|以及|应该|需要|支撑|判断|定位|差异化|价格|门店模型|会员体系|what|how|why|which|should|need|needs|and|or|the)$/i;
...
export async function deriveResearchQuestionsLLM({ brief, angles, callModel } = {}) {
  if (typeof callModel !== "function")
    throw new Error("deriveResearchQuestionsLLM requires callModel");
...
export function buildReflectionPrompt({ question, findings = [], strongSourceHint = false } = {}) {
  ...
  if (strongSourceHint) {
    systemLines.push(
      "优先建议能命中强来源的下一查询：咨询机构报告、行业协会与统计局数据、行业研究院白皮书、上市公司年报；next_queries 中至少 1 条带“报告/白皮书/研究院/统计/年报”类词。",
    );
  }
...
export async function researchQuestionWithReflection({
  ...
  if (typeof search !== "function")
    throw new Error("researchQuestionWithReflection requires search");
  if (typeof callModel !== "function")
    throw new Error("researchQuestionWithReflection requires callModel");
  const findings = [];
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
    ...
    if (round === 1 && hits.length === 0) {
      const retryQueries = buildResearchQueryVariants(question, {
        retry: true,
        maxVariants: 6,
...
function summarizeStrongSources(sources = []) {
  ...
  const strong = (sources || []).filter(
    (source) => source.source_tier === "T1" || source.source_tier === "T2",
  ).length;
  return {
...
export async function gatherResearchDeep({
  ...
  if (allFindings.length === 0) {
    const errors = perQuestion
      ...
      .join("; ");
    throw new Error(
      `No traceable research findings across questions${errors ? `: ${errors}` : ""}`,
    );
  }
