...
function classifyUrl(source, opts = {}) {
  ...
  if (clientDomains.some((domain) => hostMatches(host, String(domain).toLowerCase()))) {
    return {
      source_tier:
        /\/(investor|ir|annual|report|data|research|whitepaper|about|company|news)\b/i.test(
          url.pathname,
        )
          ? "T1"
          : "T3",
  ...
  if (
    ...
    hostMatches(host, "sec.gov") ||
    hostMatches(host, "stats.gov.cn")
  ) {
    return {
      source_tier: "T2",
      source_label: "政府/监管来源",
      type: "official_data",
      tier_inferred: false,
    };
  }
  ...
  if (
    host.endsWith(".org") &&
    /(association|institute|research|standard|industry|cipa|camera|imaging)/i.test(host)
  ) {
    return {
      source_tier: "T2",
      source_label: "行业协会/研究机构来源",
      type: "industry_report",
      tier_inferred: true,
    };
  }
...
function valueHasTraceInContent(value, content) {
  ...
  if (numberTokens.length > 0) {
    const numbersFound = numberTokens.every((token) =>
      normalizedContent.includes(normalizeForMatch(token)),
    );
    const phraseFound =
      phraseTokens.length === 0 ||
      phraseTokens.some((token) => normalizedContent.includes(normalizeForMatch(token))) ||
      (/复购率|大学毕业|收入|用户画像|忠诚|学历/.test(String(value || "")) &&
        /复购率|大学毕业|收入|用户画像|忠诚|学历/.test(String(content || "")));
...
export function extractVerifiableLocalValue(value, content) {
  ...
  if (valueNumbers.length > 0) {
    const numbered = sentences.find((line) =>
      valueNumbers.every((token) => normalizeForMatch(line).includes(normalizeForMatch(token))),
    );
  ...
  const keywordHit = sentences.find((line) =>
    valueKeywords.some((token) => normalizeForMatch(line).includes(normalizeForMatch(token))),
  );
...
export function verifyLocalDataRef(ref = {}, opts = {}) {
  ...
  if (!valueHasTraceInContent(ref.value || ref.statement || ref.title || "", content)) {
    throw new Error(
      `NO-FALLBACK violation: unverifiable local data_ref value "${ref.value || ""}" not found in ${source}`,
    );
  }
  return true;
}
...
export function sortBySourceTier(items = []) {
  return [...items].sort((a, b) => {
    ...
    return String(a.source || a.source_url || "").localeCompare(
      String(b.source || b.source_url || ""),
    );
  });
}
