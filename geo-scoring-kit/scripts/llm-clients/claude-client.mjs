...
export function isTransientLLMError(error) {
  ...
  return /(?:\b(?:408|409|425|429|500|502|503|504|520|522|524)\b|fetch failed|network|timeout|temporar|rate limit|socket|other side closed|ECONNRESET|ETIMEDOUT|UND_ERR_SOCKET)/i.test(
    `${message} ${causeCode}`,
  );
}

export async function withTransientLLMRetry(
  fn,
  {
...
async function callOpenAIChatCompletions(systemPrompt, userPrompt, opts = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      `OpenAI-compatible chat completion failed: ${response.status} ${text.slice(0, 300)}`,
    );
  }
