...
function normalizeItem(item, index) {
  ...
  if (!["text", "note", "url", "file"].includes(type))
    throw new Error(`unsupported source type: ${type}`);
  const source =
    type === "url" || type === "file" ? String(item.value || `source-${index + 1}`) : type;
...
function extractJsonObject(value) {
  ...
  if (start === -1 || end === -1 || end <= start)
    throw new Error("ingest response must contain JSON object");
  return JSON.parse(raw.slice(start, end + 1));
}

export async function ingestSources({ items = [], callModel } = {}) {
  if (!Array.isArray(items) || items.length === 0)
    throw new Error("ingestSources requires items[]");
  ...
  const user = normalized
    .map((item, index) =>
      [`# source ${index + 1}`, `type: ${item.type}`, `source: ${item.source}`, item.text].join(
        "\n",
      ),
    )
    .join("\n\n");
