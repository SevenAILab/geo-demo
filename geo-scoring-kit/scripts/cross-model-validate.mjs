...
export function evaluateOutput(text) {
  ...
  if (slides.every((slide) => slide.action_title && !/(介绍|概述|分析)$/.test(slide.action_title)))
    score += 15;
  else reasons.push("Action Title 不完整");

  if (slides.every((slide) => Array.isArray(slide.models_used) && slide.models_used.length > 0))
    score += 15;
  else reasons.push("models_used 不完整");

  if (
    slides.some(
      (slide) => Array.isArray(slide.data_refs) && slide.data_refs.some((ref) => ref.source),
    )
  )
    score += 10;
...
async function readSmallRigUserPrompt(agentId) {
  const bundlePath = path.join(
    REPO_ROOT,
    "outputs",
    agentId === "brand_positioning" ? "smallrig" : `smallrig-${suffixForAgent(agentId)}`,
    "prompt-bundle.md",
  );
  try {
...
async function runOnce(agentId, model, options = {}) {
  ...
  const systemPrompt = await fs.readFile(
    path.join(REPO_ROOT, `prompts/${agentId}/system.md`),
    "utf8",
  );
  const userPrompt = await readSmallRigUserPrompt(agentId);
  return model.fn(systemPrompt, userPrompt, {
...
export function renderReport({ matrix, models, generatedAt, dryRun = false, env = {} }) {
  ...
  for (const agent of Object.keys(matrix)) {
    ...
    for (const model of models) {
      ...
      lines.push(
        `- ${model.name}: ${result?.score ?? 0}/100${result?.error ? `, error=${result.error}` : ""}`,
      );
    }
    lines.push("");
  }
