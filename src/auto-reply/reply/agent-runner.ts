...
export async function runReplyAgent(params: {
  ...
  try {
    ...
    // LobsterAI patch: skip derivePromptSegments fallback. The fallback is only
    // used for trace/diagnostics prompt segmentation and must not block reply
    // delivery when raw user text contains media attachment markers.
    const promptSegments = runResult.meta?.promptSegments as TracePromptSegmentView[] | undefined;
